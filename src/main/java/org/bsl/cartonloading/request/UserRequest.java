package org.bsl.cartonloading.request;
...
public class UserRequest {
    ...
    /** Multipart-friendly buyer values: LL_BEAN,TNF,PATAGONA,... */
    private String buyerPermissions;
    ...
    public String getBuyerPermissions() { return buyerPermissions; }
    public void setBuyerPermissions(String buyerPermissions) { this.buyerPermissions = buyerPermissions; }
    ...
    public List<String> getBuyerPermissionList() {
        if (buyerPermissions == null || buyerPermissions.trim().isEmpty()) {
            return User.normalizeRole(role).equals(User.ROLE_ADMIN)
                    ? org.bsl.cartonloading.model.BuyerAccess.ALL
                    : new ArrayList<>();
        }
        String raw = buyerPermissions.trim().replace("[", "").replace("]", "").replace("\"", "");
        return org.bsl.cartonloading.model.BuyerAccess.normalizeAll(
                Arrays.asList(raw.split("[,;|]")),
                User.normalizeRole(role).equals(User.ROLE_ADMIN)
        );
    }
}
